        ...
        <div className="max-w-[820px] mx-auto px-6">
          ...
          <Reveal>
            <Link href="/#travaux" className="arrow-link mb-8 inline-flex" style={{ transform: "scaleX(1)" }}>
              <span style={{ transform: "rotate(180deg)", display: "inline-block" }}>→</span> {ui.caseStudyPage.back}
            </Link>
          </Reveal>
          ...
          <Reveal>
            <section className="mt-14">
              <h2 className="text-[22px] mb-5">{ui.caseStudyPage.overview}</h2>
                ...
                {cs.captures.map((c, i) => (
                  <div key={i} className="rounded-xl grid place-items-center aspect-[16/10]" style={{ border: "1px solid var(--line)", background: "linear-gradient(135deg, var(--line-soft), var(--surface))" }}>
                    <div className="text-center font-mono text-[12.5px]" style={{ color: "var(--ink-3)" }}>
                      ...
                      {c.label}<br /><span className="opacity-70">{ui.caseStudyPage.capturePlaceholder}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Reveal>
            <section className="mt-14 pt-10" style={{ borderTop: "1px solid var(--line)" }}>
              <div className="grid sm:grid-cols-[1fr_auto] gap-8 items-start">
                <div>
                  <h3 className="font-mono text-[11px] uppercase tracking-wide mb-3" style={{ color: "var(--ink-3)" }}>{ui.caseStudyPage.technologies}</h3>
          ...
          <Reveal>
            <div className="mt-14 text-center rounded-2xl p-10" style={{ background: "var(--surface)", border: "1px solid var(--line)" }}>
              <p className="text-[17px] mb-5" style={{ color: "var(--ink-2)" }}>{ui.caseStudyPage.ctaText}</p>
              <Link href="/#contact" className="btn btn-primary">{ui.caseStudyPage.ctaButton}</Link>
            </div>
          </Reveal>
        </div>
      </article>
